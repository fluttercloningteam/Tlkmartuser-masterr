...
import 'package:tlkmartuser/Screen/Map.dart';
...
class StateAddress extends State<AddAddress> with TickerProviderStateMixin {
  ...
  StateSetter? areaState, cityState, pinCodeState;
  ...
  bool cityLoading = true, areaLoading = true, pinLoading = true;
  ...
  User? selArea;  User? selPinCode;
  int? selAreaPos = -1, selCityPos = -1, selPinPos = -1 ;
  ...
  void initState() {
    ...
    _pinCodeController.addListener(() {
      pinSearch(_pinCodeController.text);
    });
  ...
  Future<void> getPinCode() async {
    try {
      ...
      if (widget.update!) {
        selPinPos = pinSearchLIst
            .indexWhere((f) => f.zipCode == addressList[widget.index!].pincode);

        if (selPinPos == -1) selPinPos = null;
      }
    } on TimeoutException catch (_) {
      setSnackbar(getTranslated(context, 'somethingMSg')!);
    }
